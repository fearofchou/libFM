...
int main(int argc, char **argv) { 
	...
	try {
		...
		const std::string param_cache_size = cmdline.registerParameter("cache_size", "cache size for data storage (only applicable if data is in binary format), default=infty");
                ...
                const std::string param_save_model = cmdline.registerParameter("save", "filename for saving model parameters");
                const std::string param_load_model = cmdline.registerParameter("load", "filename for loading model parameters");
		...
		if (cmdline.getValue(param_verbosity, 0) > 0) { 
			...
			fml->debug();			
                ...
                if (cmdline.hasParameter(param_load_model)) {
                    fm.load_model(cmdline.getValue(param_load_model));
                }
		...
		fml->learn(train, test);
                ...
                if (cmdline.hasParameter(param_save_model)) {
                    fm.save_model(cmdline.getValue(param_save_model));
                }
